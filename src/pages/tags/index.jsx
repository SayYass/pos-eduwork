...
const Tags = () => {
  ...
  const index = () => {
    axios.get('http://localhost:3000/api/tags'  ).then(res => setData(res.data)).catch(err => console.log(err))
    console.log(data)
  }
  useEffect (() => {
    index();
  ...
  const handleSubmit = () => {
    const body = {name}
    axios.post('http://localhost:3000/api/tags', body , config).then(e => console.log(e)).catch(err => console.log(err.message))
    setTimeout(()=> window.location.reload() , 2000)
  }

  const handleDelete = (_id, e) => {
    e.preventDefault();
    axios.delete(`http://localhost:3000/api/tags/${_id}`, config).then(res => console.log(res))
    setTimeout(()=> window.location.reload() , 2000)
  }
